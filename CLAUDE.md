...
- `orchestrator/drift_detectors.py` — **deterministic Phase-1 drift scanner**, runs after each post-coder cycle (wired in `pipelines/post_coder.py`). Two registries: `_DETECTORS` (comment path — findings post as `feature_comments` with `author="drift-scanner"`, deduped against the last 24h, and surface to the next coder via `{reviewer_feedback}` injection), and `_CHORE_DETECTORS` (chore path — high-severity findings get filed as Approved chore features when the `RECONCILER_CHORES_ENABLED` env flag / `product.config.reconciler_chores` is on; gated by `dedupe_key` so an open chore isn't re-filed). New detectors soak on the comment path, then promote to the chore path after a clean two-audit soak (Guard-17-tuning protocol); near-zero-FP detectors may go straight to the chore path. Detectors include `shell_artifact_files`, `design_doc_missing`, `duplicate_ddl`, `god_file`, `public_route_blanket_with_auth`, and the **wave-6 security trio** (`insecure_cors` = wildcard origin + credentials, `schema_dual_source` = table CREATEd in code but absent from migrations — both chore-path; `timing_unsafe_compare` = `==` on secret/digest material — comment-path soak). Each is a pure function `(working_dir, features) -> list[Finding]`; add new ones to the appropriate registry and a sibling test in `tests/test_drift_detectors.py`. The architect/security_auditor personas are the slow/reasoning siblings — these detectors are the cheap layer that runs every cycle.
