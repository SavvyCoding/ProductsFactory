...
- `MAX_PENDING_APPROVED` — Backlog self-throttle for the planner persona: when a product's Approved queue drains, the planner generates fresh features only up to this cap (default: 10). Resolution is `system_config.max_pending_approved` column (migration 048, hot-reloadable from Settings → Poller card) → this env var → built-in default; consumed by `orchestrator/cycle/persona.py` step 5.
