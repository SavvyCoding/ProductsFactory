...
- `orchestrator/prompts/` — per-persona prompt templates (`designer.md`, `reviewer.md`, `architect.md`, `planner.md`, `product_trainer.md`, `analytics.md`, `devops.md`, `documenter.md`, `recommender.md`, `refactorer.md`, `security_auditor.md`, `code_auditor.md`, `analysis_run.md`) + a builder (`__init__.py`) that fills in product context and injects `Pattern:` lines harvested from `session_summary.md` (capped at 10 patterns / 200 chars each to bound prompt growth). Two non-persona files: `_hard_rules.md` (meta-rules injected into every agent session — wave-10 sizing/scoping defense) and `brownfield.md` (variant coder/designer scope guidance for brownfield products). There is no `coder.md`; the coder prompt is assembled by the builder. Add a new persona prompt here; do **not** inline persona text into `docker_runner.py`.
...
- `orchestrator/agent_loop.py` — backend-agnostic tool-use loop extracted from `ollama_agent.py`. A `Backend` protocol turns one chat turn into an assistant message; a `ToolDispatcher` callable returns `(result_text, is_done)`. New backends plug in here instead of duplicating the message/tool plumbing.
- `orchestrator/backends.py` — the **premium API backend implementations** (`ClaudeAPIBackend`, `OpenAIBackend`) conforming to `agent_loop.Backend`. Used by a feature's **escalation pass** to re-run the *same* tool loop/dispatcher on a frontier model instead of Ollama; only the "prompt → assistant message + tool calls" step differs per provider. Each backend accumulates input/output token + USD cost so the daily-USD escalation cap can sum real spend. SDKs imported lazily; the agent image carries `anthropic`/`openai`. Gated by `blocked_escalation_enabled` (migration 047).
- `orchestrator/cycle/dependencies.py` — **`depends_on` dispatch-gating** (wave-10 keystone, 2026-06-13). Before this, `features.depends_on` was decorative — written by the designer and walked by the phase report, but never consulted at dispatch, so `B.depends_on == A` could run in parallel and collapse vertical slicing (canonical: IndianFoodTruck NextAuth cascade #1621). Makes `depends_on` an enforced gate via `dependency_blocked_feature_ids`, called from the SAME two enforcement points as the phase gate (`cycle/persona._decide_action` + `docker_runner._fetch_assigned_features`) so they stay consistent. Also finally holds a story whose `depends_on` points at an `infra` provisioning story until that service is provisioned.
- `orchestrator/capabilities.py` — `AGENT_IMAGE_TOOLS` frozenset: single source of truth for which CLI tools agent/test/verify containers are expected to have (mirrors `deploy/docker/Dockerfile` — update in the same commit). A test failure citing a tool IN this set → `env_broken` (transient, no `fix_attempts` bump); a tool NOT in the set (e.g. `docker`, deliberately absent) → terminal Block-for-redesign. Rendered into the designer prompt so stories aren't written against absent tools.
- `orchestrator/main_suite_health.py` — main-test-suite health monitoring; `orchestrator/fresh_env_check.py` — fresh-env install/collect probe; `orchestrator/github_client.py` — GitHub REST client wrapper (distinct from `integrations/github_app.py`, the auth chokepoint).
- `orchestrator/drift_detectors.py` — **deterministic Phase-1 drift scanner**, runs after each post-coder cycle (wired in `pipelines/post_coder.py`). Two registries: `_DETECTORS` (comment path — findings post as `feature_comments` with `author="drift-scanner"`, deduped against the last 24h, and surface to the next coder via `{reviewer_feedback}` injection), and `_CHORE_DETECTORS` (chore path — high-severity findings get filed as Approved chore features when the `RECONCILER_CHORES_ENABLED` env flag / `product.config.reconciler_chores` is on; gated by `dedupe_key` so an open chore isn't re-filed). New detectors soak on the comment path, then promote to the chore path after a clean two-audit soak (Guard-17-tuning protocol); near-zero-FP detectors may go straight to the chore path. The two registries (the `_DETECTORS` / `_CHORE_DETECTORS` tuples at the bottom of the file are the source of truth — keep this list in sync when adding one):
  - **Comment path (`_DETECTORS`)**: `detect_shell_artifact_files`, `detect_design_doc_mismatch`, `detect_placeholder_template_content`, `detect_sandbox_path_literals`, `detect_stub_confessions`, `detect_secret_sentinel` (high-severity, soaking before promotion), `detect_timing_unsafe_compare` (wave-6, `==` on secret/digest material), `detect_overlapping_features` (wave-7, in-flight features racing to co-create the same not-yet-shipped file — consolidation is a judgment call so it stays comment-only).
  - **Chore path (`_CHORE_DETECTORS`)**: `detect_duplicate_ddl`, `detect_god_file`, `detect_public_route_blanket_with_auth`, `detect_mixed_error_envelopes`, `detect_tracked_build_artifacts`, `detect_undeclared_backend_deps`, `detect_unreachable_emitted_urls` (last three promoted from comment path 2026-06-11), plus the **wave-6 security pair** `detect_insecure_cors` (wildcard origin + credentials) and `detect_schema_dual_source` (table CREATEd in code but absent from migrations) — both went straight to chore path as near-zero-FP.
  Each is a pure function `(working_dir, features) -> list[Finding]`; add new ones to the appropriate registry and a sibling test in `tests/test_drift_detectors.py`. The architect/security_auditor personas are the slow/reasoning siblings — these detectors are the cheap layer that runs every cycle.
...
- `detect_repeated_gate_rejection` (gate-loop circuit-breaker, PR #63) — a feature bounced on the *same* gate rejection (lint/test/verify) repeatedly → escalate instead of looping the coder on an unfixable gate.
- `detect_no_progress_sessions` (no-progress guard, PR #64) — a feature whose recent coder sessions repeatedly run long / burn turns but push nothing → Block it rather than let it silently churn compute.
- `detect_reviewer_outcome_text_mismatch` — reviewer's `review_outcome` field disagrees with its decision text → flag the inconsistent verdict.
- `detect_designer_bounce` — designer-starvation guard: an Approved feature (no `design_doc_path`) repeatedly claimed (→Designing) and rolled back to Approved with no doc keeps starving the queue (each bounce is 2 slow transitions, so `rapid_flap` misses it) → Block for PM review.
- `detect_dead_dependency` — dependency-deadlock guard: an Approved/Designed feature whose `depends_on` target is terminal-non-Pushed (Rejected/Reverted/Deferred) is frozen forever (the dependency gate only releases on Pushed) → Block so the deadlock is a visible, actionable row.

**Blocked-feature re-processor** (wave-8, 2026-06-13, `deploy/orchestrator/tools.py::_reprocess_blocked_features`, per-cycle, **ON by default for all products**; `BLOCKED_REPROCESSOR_ENABLED` is a global kill switch, `product.config.blocked_reprocessor=False` is a per-product opt-out): the supervisor's blockers are *preventive* — they stop new blocks but the existing Blocked pool only grows (2026-06-13 audit: 75 Blocked, class distribution unchanged across waves). This drains it by giving each Blocked feature **one** automatic retry (deduped by a `blocked-reprocessor` comment, capped 2/product/cycle) through the machinery that can now handle its class: `divergent_review` → unblock Approved + clear doc + `fix_attempts=0` (cumulative-checklist retry); `rapid_flap`/`fix_attempts`-cap with **code-quality** bounce authors (lint/test/verify/reviewer) → unblock Approved + `fix_attempts=ESCALATION_FIX_ATTEMPTS_THRESHOLD` so the next coder session runs the **diagnose-first escalation** (wave-5) — this also closes the gap where `rapid_flap` blocks on transition-count at `fix_attempts=3`, *before* the escalation trigger (≥4) fires. **Stale test-gate timeout** blocks (2026-07-20, `_stale_test_gate_env_block`) → unblock Approved + `fix_attempts=0`: an `env_impossible` post-coder *test-gate timeout* whose cited budget is below the product's **current** resolved gate budget (`_resolve_test_gate_timeout`) is the one env exception — the cited budget is the **largest** seconds figure found in the diagnosis text, and a diagnosis with **no explicit seconds citation never qualifies** (commit `0f9b244`; taking the smallest / assuming a default budget both produced spurious retries) — the environment itself changed (the gate self-calibrates up as green-run samples accumulate via `config.test_gate_runtimes`), so unlike other env blocks a re-run CAN now pass (canonical: DogTinder product 31 — 6 features blocked citing a 300s gate that grew to the 1800s ceiling). Only the *timeout* class qualifies; service/tool-missing, verify-recipe, and read-only-file env blocks are NOT gate-timeouts and stay skipped. Other `env`/`service`/`tool`/`spec` blocks are skipped (a coder re-run won't help; the service/tool-missing + `spec_defect` paths own those).

**Diagnose-first escalation** (wave-5, hardened wave-9, 2026-06-13): a coder session for a feature at `fix_attempts >= ESCALATION_FIX_ATTEMPTS_THRESHOLD` (default 4) runs as a **read-only diagnostician** — `write_file` and git-mutating bash are blocked **at the tool layer** (`ollama_agent.py` honours `AGENT_ESCALATED=1`, same enforcement as `_READONLY_PERSONAS`), so the agent *must* produce a verdict instead of another blind code attempt. The wave-5 version was prompt-only and the qwen3-coder model ignored it — it just coded and re-blocked (proven on #1591/#1621/#1542); wave-9 makes the diagnose-first contract structural. The session reads the bounce history + failing gate and posts `ESCALATION-DIAGNOSIS: <fixable|spec_defect|env_impossible> — <root cause>`; `docker_runner._route_escalation_diagnosis` then routes: `spec_defect` → designer (doc cleared), `env_impossible` → Blocked-with-diagnosis, `fixable` → no-op (the feature stays coder-eligible and the **next** session is a normal writeable coder that applies the fix with the diagnosis in `{reviewer_feedback}`). Runs **once** per feature (`_has_escalation_diagnosis` gate — a feature with a verdict gets the normal coder, not another diagnostician); the read-only session writes no code, so it skips the post-coder commit/push/auto-heal pipeline. Claude-CLI backend can't tool-gate, so escalation stays prompt-only there (production is Ollama). **Premium-model routing** (migration 047, `blocked_escalation_enabled`): when enabled, the escalation pass (the `fixable` follow-up coder) runs on a frontier model via `orchestrator/backends.py` instead of Ollama, under a daily-USD cap that sums the per-backend cost counters. The flag is re-checked every run (commit `938f3e1`), so toggling it takes effect without a restart.

**Coder model-escalation ladder** (migrations 049/050, `orchestrator/coder_tiers.py`, merged `1cc1cae`) generalizes the single-flag premium routing above into an **ordered ladder** of model tiers configured in `system_config.coder_tiers`: a First Attempt plus up to three escalation tiers, each a `(backend, model, max_attempts, enabled)` where `backend ∈ {ollama, claude-api, openai}`. A *diagnostician-judged `fixable`* feature climbs one tier at a time; `env`/`spec` verdicts never climb. `coder_tiers.py` is **pure/side-effect-free** (no DB/Docker imports — unit-tests token-free) and owns just the arithmetic: `effective_tiers()` (First Attempt is always live regardless of its `enabled` flag; disabled escalation tiers are dropped so `escalation_step` still maps onto a real tier) and tier resolution from `features.escalation_step`. **`escalation_step` is a dedicated *cumulative* counter** (total failed ladder attempts) — deliberately NOT `fix_attempts` (which the blocked re-processor resets); the active tier is derived from `max(escalation_step, fix_attempts)` per `92af092`. Walking each live tier's cumulative `max_attempts` maps the one counter onto per-tier budgets (e.g. `[minimax×2, glm×3, opus×2]` → steps 0–1 minimax, 2–4 glm, 5–6 opus, 7+ `EXHAUSTED`). Exhausting the last live tier yields the `EXHAUSTED` sentinel → the feature routes to the normal `Blocked` holdpen (not a new terminal state). Paid backends (`claude-api`/`openai`, `PAID_BACKENDS`) sum against the daily-USD cap; ollama is free. Configured from the **"🧠 Model Selection" admin tab** (consolidated from separate designer/coder cards in `c983eb7`), exposed via `/api/system-config` (`coder_tiers`, `4d9f142`+`863490c`). Migration 050 adds durable ladder-escalation telemetry. The web image must bundle `coder_tiers.py` (`4d9f142`) — it is web-imported (see the selective-COPY gotcha). Design doc: `docs/design/coder_escalation_tiers.md`.

### Session continuity (`session_summary.md`)

`session_summary.md` is the per-product cross-session continuity doc, replacing the retired `progress.md` (2026-05-28, commit `9fa045f`). `orchestrator/docker_runner.py::_read_session_summary` reads it before each session and passes the contents into the agent prompt as `{prev_session_summary}`; on session end it's committed by `integrations/git_ops.py` so the next session on the same product sees the freshest notes. Reads are tail-keep capped (commit `b58bc8d`) so the newest notes survive the prompt-size limit, and **sanitized of NUL/C0 control bytes** (commit `3a9c771`): a single corrupt byte written by a truncated container (canonical: `"89.50\x00overage"`) flowed into `{prev_session_summary}` → the docker CLI args → `ValueError: embedded null byte` on every subsequent launch, wedging the product. `docker_runner._sanitize_cmd_args` strips control bytes from all docker cmd args as belt-and-suspenders (tab/newline/CR kept). Reviewer-authored `Pattern:` lines inside it are also harvested by `orchestrator/prompts/__init__._read_reviewer_patterns` and injected separately into coder/designer prompts as a checklist (capped at 10 / 200 chars). If you're tempted to add a new long-lived continuity file, extend this one instead.
...
**`security_auditor`** (wave-6, 2026-06-13) is a **read-only, PM-triggered, product-wide security audit** that files findings as `bug` features (`priority=10`, ≤8/session) via the PM API — it writes no repo files (enforced by `ollama_agent._READONLY_PERSONAS`) and is kept OUT of `_MAINTENANCE_PERSONAS` (no commit pipeline). It rides the on-demand path (`_ONDEMAND_PERSONAS`, the "🔒 Security audit" button on the product page → `run_persona_now`). **As of 2026-07-16 it also rides the phase-review gate on the architect-run cadence** (promoted from PM-button-only): `_run_phase_review_gate` queues it via its own counter `config.architect_runs_at_last_security_audit`, gated by `_security_auditor_scheduled_enabled` — **always-on, env-only** (`SECURITY_AUDITOR_SCHEDULED_ENABLED`, default ON; the per-product opt-out and UI checkbox were removed 2026-07-16). The PM button is unchanged. Security ownership moved fully here when the security dimension was dropped from `code_auditor` (below), so it's the single focused security pass rather than one-third of a shared review. This is distinct from and additive to the per-PR security review that was merged into `reviewer` on 2026-05-06 (that merge is unchanged); `security_auditor` is the periodic whole-product sweep that the per-PR reviewer can't be. Prompt: `orchestrator/prompts/security_auditor.md`. Together with the deterministic drift detectors (cheap layer), it closes the loop the 2026-06-13 DogTinder re-review surfaced: semantic security findings now become filed features instead of dying in a comment.

**`code_auditor`** (2026-06-24, design doc `docs/design/code_auditor_review_gate.md`) is a **read-only, phase-triggered, whole-product semantic review gate** — a third persona category alongside `_MAINTENANCE_PERSONAS` (scheduled) and `_ONDEMAND_PERSONAS` (PM-button). It rides the on-demand path (`code_auditor` is in `_ONDEMAND_PERSONAS`) but is *queued automatically* by `deploy/orchestrator/tools.py::_run_phase_review_gate` at a phase boundary, on an **architect-run cadence** (`CODE_AUDIT_ARCHITECT_RUNS`, default 1 during soak / 2 steady ≈ every 6 features) so it always reads the freshest `ARCHITECTURE.md` and dedups against the architect's just-filed chores. It reviews the whole product with `ARCHITECTURE.md` context injected, across **two semantic dimensions — correctness · tests** (security moved to `security_auditor` on 2026-07-16; architecture/duplication is the architect's job), with a refute-before-file self-critique. Both auditors ride the SAME `_run_phase_review_gate` on the architect-run cadence, each with its own counter, serialized by `run_persona_now` (if both are due, one fires this cycle, the other next). Read-only (in `ollama_agent._READONLY_PERSONAS`, skips the commit pipeline). Why it exists: the per-PR `reviewer` sees one session diff and the drift detectors are pattern matchers — neither catches cross-cutting accretion (parallel Stripe modules, committed DBs) or semantic security/correctness bugs (IDOR, payment-never-persisted) that only a whole-product semantic pass finds.
- **Rollout is staged and gated.** The audit is **ALWAYS ON for every product** as of 2026-07-16 — the per-product `product.config.code_auditor` opt-out and its Workflow-card checkbox were removed; the only off-ramp is the global `CODE_AUDITOR_ENABLED` kill switch (default ON). (Earlier it was a comment-only soak behind `CODE_AUDITOR_ENABLED` + `product.config.code_auditor`, default OFF.) Findings → dashboard `alerts`, `[code-audit]` prefix in comment-only mode. **Filing promotion Increment 1** (SHIPPED 2026-06-25, behind `CODE_AUDITOR_FILING_ENABLED` / `product.config.code_auditor_filing`, default OFF): each surviving finding files as a `bug` feature severity-routed by priority (Critical→1, High→5, Medium→20, Low→40) and, as of 2026-07-16, **severity-routed by status**: Critical/High land `Approved` (auto-triaged straight into the coder queue for fast repair), Medium/Low land `Pending` (PM triages first). `security_auditor` filings follow the same status rule (priority stays flat 10). Trades the human gate for speed on the top two tiers; the Increment-2 verifier is the eventual compensating control. **Increment 1.5** auto-attaches each filed finding to a standing per-product **"Code-Review Hardening"** phase (`website/main.py::_get_or_create_hardening_phase`) so audit bugs don't sit un-phased. The output sink (alert vs filing) is a deterministic builder-side switch (`{code_auditor_output_steps}` in `orchestrator/prompts/__init__.py`), not an agent branch, so a local model can't do the wrong half. Increment 2 (independent verifier session + deterministic blocker gate `open Critical/High == 0`) and Increment 3 (semantic-cosine dedup + `phases.review_state` migration) are deferred. Enable helpers: `_code_auditor_enabled` (now env-only, always-on default — ignores `product`) governs BOTH code_auditor and, via `_security_auditor_scheduled_enabled`, the security sweep that rides the same `_run_phase_review_gate`; `_code_auditor_filing_enabled` (still an explicit per-product bool, wins over env) governs comment-only-vs-filing. Prompt: `orchestrator/prompts/code_auditor.md`; tests: `tests/test_code_auditor_gate.py`, `tests/test_code_auditor_filing.py`.

**Spec-and-filing guards (2026-07-27)** — three guards that stop the "not-real-work" blocked-pool churn (broken ACs, stale-duplicate + false-positive audit bugs). All env-flagged, default-safe:
- **Fix 1 — designer AC-recipe self-check** (`orchestrator/pipelines/post_doc.py::_post_doc_ac_recipe_check`): statically validates every `Verify:` recipe in a staged `docs/story_<id>.md` (shell syntax via `bash -n`, `Expected:` JSON well-formedness) so a syntactically-broken recipe never reaches a coder (canonical: MyGroceryApp #3196, AC3 unclosed brackets → coder flapped to fix_attempts=7). Reuses `post_coder._parse_ac_verifies`. `DESIGNER_AC_LINT_ENABLED` (default ON); ADVISORY by default (comment, ship the doc), `DESIGNER_AC_LINT_ENFORCE=1` promotes to a hard bounce. Tests: `tests/test_post_doc_ac_recipe_check.py`.
- **Fix 2a — audit-finding dedup** (`website/main.py::_find_duplicate_audit_bug`, at `POST /api/features`): when an AI files a `bug` (source=ai), skip it if a non-terminal-rejected `bug` for the product shares a `file:line` token or an identical significant title — returns the existing feature (idempotent). Catches re-filing an already-open or already-shipped finding (canonical: #3179 re-filed #3110's merged bulk-cap; #3195/#3196 re-filed already-fixed bugs). `AUDIT_DEDUP_ENABLED` (default ON, web block). Tests: `tests/test_website.py::TestAuditFindingDedup`.
- **Fix 2b — independent audit verifier** (persona `audit_verifier`, `orchestrator/prompts/audit_verifier.md`; gate `deploy/orchestrator/tools.py::_run_audit_verifier_gate`): read-only pass that re-checks newly-filed audit bugs against the code on a **different model** (`AUDIT_VERIFIER_MODEL`, else the designer model — distinct from the coder/auditor) and Rejects false positives (canonical: #3131, #3203). Queued when **ANY** open audit bug lacks an `audit-verifier` verdict comment (the dedupe marker) — the gate scans newest-first and short-circuits on the first unverified. (Originally it checked only the *newest* bug; the verifier session processes oldest-first and caps at ~8/session, so with >8 open bugs the newest was never reached and the gate re-queued the verifier every cycle — fixed `3fbb39d`. Checking "any unverified" converges: each run drains up to 8.) In `_ONDEMAND_PERSONAS` + `_READONLY_PERSONAS`. `AUDIT_VERIFIER_ENABLED` env (default ON) with per-product opt-out `product.config.audit_verifier=False`. Tests: `tests/test_audit_verifier_gate.py`. This is the deferred code_auditor "Increment 2 verifier," generalized to both auditors.
...
- Model-telemetry columns on `sessions` (migrations 050–052): `ladder_model` (coder-only — the escalation-ladder tier that ran), **`model`** (051 — the resolved PRIMARY model for *every* persona's session; before this, non-coder sessions bucketed under bare `backend` and designer/reviewer models never appeared in the dashboard), **`model_requests`** (052 — JSONB `{model: count}` of the model that ACTUALLY served each request, accumulated by `ollama_agent` and PATCHed at session end; fallback-aware, so when the primary is busy and requests silently fall back to the 2nd-choice model the dashboard still reconciles with Ollama's own request counts). The dashboard's Model Usage panels group by `COALESCE(model, ladder_model, backend)`.
...
- **Git auth: GitHub App only.** All git push, PR API calls, and repo creation go through short-lived installation access tokens minted by `orchestrator/integrations/github_app.py` (JWT signed with the App's PEM → POST `/app/installations/{id}/access_tokens` → 60-min token, cached and refreshed when < 5 min remains). The JWT is signed for **9 minutes, not GitHub's 10-minute max** (`_JWT_LIFETIME_SECONDS = 540`, commit `8074fa3`): `exp = now+600` left zero skew margin, and Docker Desktop VM clock drift produced 401 "exp too far in future" that took ALL git auth down (2026-08-08 incident); `iat` is also back-dated 60s. Don't "fix" it back to 600. App config lives in `system_config` (`github_app_id`, `github_app_private_key`, `github_app_installation_id`, `github_org`) — read fresh each call so operators can rotate without restarting the poller. The PM website was migrated off direct `github_pat` reads in `6d16deb`; do not propose PAT fallbacks. SSH deploy keys (per-product `id_ed25519_{name}` and `id_ed25519_productfactory`) are retired.
...
- `MAX_PENDING_APPROVED` — Backlog self-throttle for the planner persona: the planner only runs when the product's **Pending + Approved** backlog is below this cap (default: 10). Counting Pending too (commit `b60edd0`) matters — gating on Approved alone let the planner keep generating while a pile of un-triaged Pending features sat waiting on the PM. Resolution is `system_config.max_pending_approved` column (migration 048, hot-reloadable from Settings → Poller card) → this env var → built-in default; consumed by `orchestrator/cycle/persona.py` step 5.
- `RECONCILER_CHORES_ENABLED` — **global kill switch** for the corrective-chore sink (high-severity findings from `orchestrator/drift_detectors.py::_CHORE_DETECTORS` filed as Approved chore features instead of comment-only). **ALWAYS ON for all products** — the per-product `product.config.reconciler_chores` opt-out and its Workflow-card checkbox were removed 2026-07-16; this global env flag (default ON; set a falsy value `0`/`false`/`no`/`off` to disable the whole environment) is the only off-ramp. Resolution lives in `post_coder.py`'s drift block; `file_corrective_chores` caps at 3/product/cycle and dedupes open chores so steady-state filing is bounded.
