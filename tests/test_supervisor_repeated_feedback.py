...
# ── Layer 4: detect_invalid_status_combos sweep ──────────────────────────────


def _build_combo_sweep_client(*, products: list[dict],
                              features_by_product: dict[int, list[dict]],
                              captured_patches: list[dict],
                              sysconfig: dict | None = None):
    """Mock transport for the per-cycle sweep. Endpoints:
      - GET /api/products → returns `products`
      - GET /api/products/<id>/features → returns features_by_product[id]
      - PATCH /api/features/<id> → captures into captured_patches
      - GET /api/system-config → returns sysconfig
      - POST /api/supervisor/actions → no-op
    """
    sysconfig = sysconfig if sysconfig is not None else {
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        m = path.split("/")
        if method == "GET" and len(m) == 5 and m[1] == "api" and m[2] == "products" and m[4] == "features":
            return httpx.Response(200, json=features_by_product.get(int(m[3]), []))

        # PATCH /api/features/<id>
        if method == "PATCH" and path.startswith("/api/features/"):
            captured_patches.append({
                "feature_id": int(path.rsplit("/", 1)[-1]),
                "body":       _json.loads(request.content) if request.content else {},
            })
            return httpx.Response(200, json={"ok": True})
...
class TestDetectInvalidStatusCombos:
    def test_sweep_fixes_hybrid_features(self, monkeypatch):
        """Two features in hybrid combo + one healthy feature ⇒ exactly
        the two hybrids get PATCHed to Implementing+changes_requested."""
        products = [{"id": 8, "status": "ready"}]
        features = [
            # Two invalid hybrid combos:
            {"id": 374, "status": "Reviewed",  "review_outcome": "changes_requested"},
            {"id": 377, "status": "Reviewing", "review_outcome": "changes_requested"},
            # One healthy feature that must be left alone:
            {"id": 372, "status": "Reviewed",  "review_outcome": "approved"},
            # Already-correctly-Implementing+changes_requested (idempotent):
            {"id": 100, "status": "Implementing", "review_outcome": "changes_requested"},
        ]
        captured: list[dict] = []
        transport = _build_combo_sweep_client(
            products=products,
            features_by_product={8: features},
            captured_patches=captured,
        )
        # Patch supervisor.httpx.Client to use our mock transport.
        real_client = httpx.Client
        monkeypatch.setattr(supervisor.httpx, "Client",
                            lambda *a, **kw: real_client(*a, transport=transport, **kw))

        result = supervisor.detect_invalid_status_combos(products)
        assert result["fixed"] == 2
        assert result["errors"] == 0

        # Exactly the two hybrid features were patched.
        fixed_ids = sorted(p["feature_id"] for p in captured)
        assert fixed_ids == [374, 377]
        for p in captured:
            assert p["body"]["status"] == "Implementing"
            assert p["body"]["review_outcome"] == "changes_requested"
            assert p["body"]["changed_by"] == "supervisor.invalid_status_combos"

    def test_sweep_disabled_is_noop(self, monkeypatch):
        products = [{"id": 8, "status": "ready"}]
        features = [{"id": 374, "status": "Reviewed", "review_outcome": "changes_requested"}]
        captured: list[dict] = []
        transport = _build_combo_sweep_client(
            products=products,
            features_by_product={8: features},
            captured_patches=captured,
            sysconfig={"supervisor_invalid_combos_enabled": False,
                       "supervisor_dry_run_only": False},
        )
        real_client = httpx.Client
        monkeypatch.setattr(supervisor.httpx, "Client",
        ...
        result = supervisor.detect_invalid_status_combos(products)
        assert result.get("skipped") == "disabled in system_config"
        assert captured == []

    def test_sweep_dry_run_does_not_patch(self, monkeypatch):
        products = [{"id": 8, "status": "ready"}]
        features = [{"id": 374, "status": "Reviewed", "review_outcome": "changes_requested"}]
        captured: list[dict] = []
        transport = _build_combo_sweep_client(
            ...
            features_by_product={8: features},
            captured_patches=captured,
            sysconfig={"supervisor_invalid_combos_enabled": True,
                       "supervisor_dry_run_only": True},
        )
        real_client = httpx.Client
        monkeypatch.setattr(supervisor.httpx, "Client",
        ...
        result = supervisor.detect_invalid_status_combos(products)
        assert result["fixed"] == 1     # would-have-fixed
        assert captured == []           # but no PATCH actually fired
