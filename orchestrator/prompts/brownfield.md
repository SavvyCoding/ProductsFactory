...
3. **Run tests scoped to the files you changed first**, then run the full suite once before `task_done`. Scoped first (e.g. `pytest tests/test_<feature>.py -q`) is fast feedback while you iterate. Full suite second (`pytest -q --no-header` from `/workspace`) catches cross-module regressions — the post-coder gate runs the full suite anyway, and if a test you didn't touch goes red there, you bounce with `fix_attempts++` (canonical 2026-05-30 DocumentSign failures: changed metrics module, broke `tests/test_health_unit.py::test_readiness_all_healthy` which nobody scoped-ran). Skip the full-suite step only if it provably exceeds the turn budget on this product — note that decision in `session_summary.md`. If a previously-passing test now fails: investigate, fix or revert. If stuck after 2 attempts, write `BLOCKED: <reason>` to `/workspace/session_summary.md` and exit cleanly.
   ...
   **Making a test pass means changing CODE — never deleting/`.skip`-ing the test or weakening its asserts to go green.** A pre-existing test that turns red is signal your change broke a contract: fix the code or revert your change. Deleting the failing test is the cheapest wrong path and an automatic reject — the lint-guard flags "test deleted with no replacement" and the reviewer re-flags the lost coverage (canonical: a coder deleted `tests/orders.test.ts`'s 4 integration tests to clear a red suite). The same applies to commenting a test body out or replacing a real assert with `assert True`.
...
- [ ] **HARD STOP: the IMPLEMENTATION is real, not a stub that satisfies its own test.** Distinct from the hollow-*test* rule above — this is about the production code. Banned: a function body that is `return <hardcoded constant>` / `return "pi_test_" + random()` / echoes its input back, paired with a test that asserts that same constant. This "circular" shape (test verifies a mock stub) is the single most common reviewer rejection. If an AC needs an external system you can't reach in-session (Stripe, an SMTP server, a 3rd-party API), call the **real** client against a sandbox/test credential or an in-memory fake of the real library — do NOT replace the function body with a constant. Tell: if you could delete the test and then delete the implementation with no other caller noticing, you shipped a stub. Mark the feature `Blocked` (with the literal blocker) before shipping a constant-returning stub.
- [ ] **The feature is wired end-to-end and fails closed.** Per-AC checks pass in isolation, but the reviewer also rejects code that is dead at runtime. Confirm: (1) the code you wrote is actually reached by the real entry point (route handler, job, middleware) — not merely unit-callable; (2) if it depends on seed/reference data to do anything (a lookup table, default rows, a config row), you seeded it — an empty table that makes the feature return `null`/`[]` is a silent failure; (3) on a critical-path failure (payment declined, auth/signature missing, downstream error) the operation **stops and surfaces the failure** — never `catch → log → continue as success`. Canonical rejections: a GPS table with zero seed rows so the map never renders; an order marked `accepted` after the payment intent threw and was swallowed.
...
- [ ] **No SQL built from request-controlled identifiers.** Values ALWAYS go through parameter placeholders (`?` / `$1` / named binds), never string interpolation. **Column and table names cannot be parameterized** — so never build an UPDATE/INSERT/ORDER BY by looping over request-body keys and splicing them into the statement (`SET ${key} = ?`, `ORDER BY ${col}`). Whitelist the columns/sort-fields a handler may touch against a fixed allow-list and reject anything not on it. Canonical cross-product reject: `updateMenuItem` / `updatePromoCode` iterated over every request-body key and interpolated it as a column name → authenticated SQL injection. This holds even on admin-only routes (defense in depth).
