...
- [ ] **Runtime tool availability — verify FIRST, before writing the test.** If any AC requires an external runtime executable (`chromium` / `playwright`, `pre-commit`, `locust`, `ruff` CLI, `eslint`, `docker-compose`, `psql`, anything spawned via `subprocess.run([tool, ...])`), step 1 of implementation is to confirm the tool exists in the agent image: `shutil.which("tool")` AND a smoke `subprocess.run(["tool", "--version"], check=True)`. If the tool is genuinely missing, mark the feature `Blocked` in `session_result.json` with `"blocked_reason": "runtime tool <name> unavailable in agent image: <stderr from probe>"` — do NOT ship a test that calls `pytest.skip()` inside the test body when the tool isn't there. In-body skips bypass the lint-guard's top-level `@pytest.mark.skip` detector but the reviewer catches them every time ("test skips when subprocess fails — design doc Verify expects the actual invocation"). The supervisor's `divergent_review_feedback` detector then auto-blocks the feature after 3 rework rounds because each round flags a DIFFERENT surface bug in the skipped-vs-real-call dance (canonical 2026-06-01 cascade: features 1082 pre-commit, 1089 Playwright infra, 1090/1091/1095 Playwright e2e tests). The Blocked-with-clear-reason path is faster, cheaper, and gets PM eyes on the missing-tool issue immediately.
